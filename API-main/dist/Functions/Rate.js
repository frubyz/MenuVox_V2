...
function rate(req, res, localPath) {
    ...
    if (!fs_1.default.existsSync(localPath + `rates/${month}/${day}.json`)) {
        if (!fs_1.default.existsSync(localPath + `rates/${month}`)) {
            fs_1.default.mkdirSync(localPath + `rates/${month}`);
        }
        fs_1.default.writeFileSync(localPath + `rates/${month}/${day}.json`, JSON.stringify([]));
    }
    ...
    fs_1.default.writeFileSync(localPath + `rates/${month}/${day}.json`, JSON.stringify(Rates));
}
